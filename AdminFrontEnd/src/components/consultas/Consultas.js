...
const Consultas = () => {
                ...
                <strong>
                    ...
                    {params.value === "Abierto" ?
                        <Button
                            ...
                            onClick={() => handleAnswer(params.row.id, params.row.mail, params.row.request)}
                        >
                            Responder
                        </Button> : null}
    ...
    const handleAnswer = (idRequest, mail, bodyText) => {
        const preBody = "Hola, te contactamos de IADS. Como estas? Recientemente, nos contactaste por la siguiente consulta:%0D%0A";
        window.location = "mailto:" + mail + "?subject=Consulta%20IADS&body=" + preBody + bodyText;
